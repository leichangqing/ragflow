...
class ESConnection(DocStoreConnection):
    ...
    def insert(self, documents: list[dict], indexName: str, knowledgebaseId: str = None) -> list[str]:
        ...
        for _ in range(ATTEMPT_TIME):
            try:
                ...
                if re.search(r"False", str(r["errors"]), re.IGNORECASE): # 检查响应应的响应中是否包含 "False"，如果包含则说明有失败的文档，需要处理这些失败的文档。
                    return res

                for item in r["items"]: # 遍历响应中的 items 列表，检查每个操作的结果
                    for action in ["create", "delete", "index", "update"]:
                        if action in item and "error" in item[action]: # 如果某个操作失败（即包含 error 字段），将失败的文档ID和错误信息添加到 res 列表中
    ...
    def update(self, condition: dict, newValue: dict, indexName: str, knowledgebaseId: str) -> bool:
        ...
        if "id" in condition and isinstance(condition["id"], str): # 如果条件中包含id且id是字符串, 则认为是要更新单个指定文档
        ...
        # update unspecific maybe-multiple documents, 处理多个未指定文档
        bqry = Q("bool")
        for k, v in condition.items():
            ...
            if k == "exists": # 则添加一个exists查询, 检查字段是否存在
                ...
                continue
            if isinstance(v, list): # 如果值是列表, 则添加一个terms查询, 匹配字段的多个值
                bqry.filter.append(Q("terms", **{k: v}))
